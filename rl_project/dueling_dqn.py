...
class DistributionalDuelingDQN(nn.Module, StateQFunction):
    ...
    def __init__(
        ...
        input_feats = 1024
        self.main_stream = nn.Linear(input_feats, 1024)
    ...
    def forward(self, x):
        # EDIT: channels first approach
        x = x / 255.0
        h = x.permute(0, 3, 1, 2)  # NHWC => NCHW
        for layer in self.conv_layers:
            h = self.activation(layer(h))

        # Advantage
        batch_size = x.shape[0]
        h_input = h.reshape(batch_size, -1)
        # print(h_input.shape)
        
        act_input = self.main_stream(h_input)
        h = self.activation(act_input)
