...
from dueling_dqn import DistributionalDuelingDQN
...
def train(config, agent, train_env, test_env, model_dir):
    ...
    logger.info('N Ops per Update:', n_ops_per_update)
    logger.info('Max Steps:', max_steps)
    ...
    for step_cnt in range(max_steps):
        ...
        assert agent.training
        agent.n_updates = agent.optim_t
        num_a2c_updates = agent.n_updates // n_ops_per_update
        ...
        if (step_cnt + 1) % config.nsteps == 0:
            ...
            logger.logkv('num_a2c_update', num_a2c_updates)
            ...
            # if num_a2c_updates % config.save_interval == 0:
            # Save every <save_interval> nsteps, 
            # i.e. save_interval=100, nsteps=5 => save every 500 steps
            
            if (step_cnt + 1) % config.save_interval * config.nsteps == 0:
                model_path = os.path.join(
                    model_dir, 'model_{}.pt'.format(num_a2c_updates + 1))
...
def run():
    ...
    # Noisy nets
    pnn.to_factorized_noisy(q_func, sigma_scale=configs.noisy_net_sigma)
    # Turn off explorer
    ...
    update_interval = configs.nsteps # originally 4
    betasteps = configs.max_steps / update_interval
    ...
    sac_agent = Agent(
        ...
        explorer=explorer,
        minibatch_size=32,
        replay_start_size=configs.replay_start_size,
    ...
    train(configs, sac_agent, train_venv, valid_venv, log_dir)
