...
def _run_episodes(
    ...
    while not terminate:
        ...
        if reset:
            ...
            # print(
            #     f"evaluation episode {len(scores)} length:{episode_len} R:{test_r}"
            # )
...
def _batch_run_episodes(
    ...
    for i, (epi_len, epi_ret) in enumerate(
        ...
        # print("evaluation episode %s length: %s R: %s", i, epi_len, epi_ret)
...
def save_agent(agent, t, outdir, logger, suffix=""):
    ...
    print("Saved the agent to %s", dirname)
...
class Evaluator(object):
    ...
    def evaluate_and_update_max_score(self, t, episodes):
        ...
        if mean > self.max_score:
            ...
            print("The best score is updated %s -> %s", self.max_score, mean)
            self.max_score = mean
    ...
    def evaluate_and_update_max_score(self, t, episodes, env, agent):
        ...
        with self._max_score.get_lock():
            if mean > self._max_score.value:
                ...
                print("The best score is updated %s -> %s", self._max_score.value, mean)
